 ...
 *   app        the kitchen sink, embedded from kitchen-sink.html
 *
...
const LAYOUT_IDS = ['master-detail', 'dashboard', 'record-wizard', 'settings-workspace'];

const KITCHEN_SINK_URL = 'kitchen-sink.html';
...
async function buildRegistry() {
  ...
  const kitchenSink = {
    ...
    id: 'invoices',
    title: 'Invoices',
    blurb: 'A complete ZeyOS screen: schema-driven table, server filtering and sorting, paging, '
      + 'and a dialog editor — generated by zx-zeyos from a live @zeyos/client schema.',
    group: 'ZeyOS',
    kind: 'app',
    url: KITCHEN_SINK_URL,
    source: KITCHEN_SINK_URL
  };

  const all = [...guides, ...components, ...layouts, kitchenSink];
...
function renderNavigation() {
  for (const section of sections) {
    ...
    for (const [group, items] of section.groups) {
      // A single-group section (Layouts, Kitchen sink) needs no second-level label.
  ...
  if (entry.kind === 'app') {
    preview.append(
      element('p', 'docs-panel__note', undefined,
        text('The kitchen sink runs in its own document so it can use the full application '
          + 'viewport. '),
        link(entry.url, 'Open it in a new tab', { target: '_blank' })),
      frame(entry.url, entry.title)
    );
    tabs.push({ id: 'preview', label: 'Preview', panel: preview });
  } else {
    entry.mount(preview);
...
/**
 * Fetches an entry's source and renders it as a highlighted, copyable code block. HTML entries
 * (the kitchen sink) are reduced to their module script, which is the interesting part.
 * @param {object} entry
...
async function loadSource(entry, panel) {
  ...
  try {
    ...
    const isHtml = entry.source.endsWith('.html');
    const code = isHtml ? extractModuleScript(raw) : raw;
    const path = `website/${entry.source.replace(/^\.\//, '')}`;
    const note = isHtml
      ? `The module script of ${path} — the page shell around it is plain HTML.`
      : 'This is the exact module the preview above just executed.';
    panel.replaceChildren(
      element('p', 'docs-panel__note', note),
      codeBlock(code, path)
    );
  } catch (error) {
...
/**
 * Pulls the body of the first `<script type="module">` out of an HTML document, de-indented.
 * @param {string} html
 * @returns {string}
 */
function extractModuleScript(html) {
  const match = /<script\b[^>]*type=["']module["'][^>]*>([\s\S]*?)<\/script>/i.exec(html);
...
/** @param {string} src @param {string} title @returns {HTMLIFrameElement} */
